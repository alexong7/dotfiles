---
name: sop-eval
description: Conversational evaluation framework for AI agents using Strands Evals SDK - plan, generate test data, execute evaluations, and analyze results.
...
8. **IMPORTANT**: Follow all HTML comment instructions in the template when generating content, then remove these comment instructions from the final report - they are template guidance only and should not appear in the generated report.
